...
trait IBackwardFlowFunction {
   ...
   */
  def callFlow(callSite: Statement, fact: Val, callee: Method, calleeSp: Statement): Collection[Val]
/**
 ...
 *     Node[Edge,Val]).
 */
def normalFlow(edge: Edge, fact: Val): Collection[State]
 ...
 *     Node[Edge,Val])
 */
def callToReturnFlow(edge: Edge, fact: Val): Collection[State]
