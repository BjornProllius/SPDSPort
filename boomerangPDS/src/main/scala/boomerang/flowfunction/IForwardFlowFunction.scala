...
import boomerang.{ForwardQuery, scene}
import boomerang.scene.{ControlFlowGraph, Field, Method, Statement, Val}
...
trait IForwardFlowFunction {
   ...
   *     method.
   */
  def returnFlow(callee: Method, returnStmt: Statement, returnedVal: Val): Collection[Val]
   ...
   *     method callee
   */
  def callFlow(callSite: Statement, factAtCallSite: Val, callee: Method): Collection[Val]
  ...
  def normalFlow(query: ForwardQuery, edge: Edge, fact: Val): Collection[State]
   ...
   *     edge.getTarget() is any successor
   * @param fact The fact that by-passes the call site.
   * @return A set of data-flow states (states in the pushdown system, typically of type
   *     Node[Edge,Val])
   */
  def callToReturnFlow(query: ForwardQuery, edge: Edge, fact: Val): Collection[State]
