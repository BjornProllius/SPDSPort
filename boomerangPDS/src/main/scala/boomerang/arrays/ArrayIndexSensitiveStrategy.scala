...
class ArrayIndexSensitiveStrategy[W <: Weight] extends ArrayHandlingStrategy[W] {

  override def handleForward(curr: ControlFlowGraph.Edge, arrayBase: Pair[Val, Int], out: Set[State]): Unit = {
    out += new PushNode[State, Field](curr, arrayBase.getX, Field.array(arrayBase.getY), PDSSystem.FIELDS)
  }
