...
abstract class WeightedPAutomaton[N <: Location, D <: State, W <: Weight] extends LabeledGraph[D, N] {
    ...
    protected def computeDistance(trans: Transition[N, D]): Int = {
        ...
        if (isUnbalancedState(trans.target)) {
            distance = Some(0)
        } else {
            distance = stateToDistanceToInitial.get(trans.target)
            if (distance.isEmpty) {
                return -1
            }
        }
        val integer: Int = distance.get + 1
        val currDistance: Option[Int] = stateToDistanceToInitial.get(trans.start)
        if (currDistance.isEmpty || integer < currDistance.get) {
            stateToDistanceToInitial.put(trans.start, integer)
            return integer
        }
        currDistance.get
    }
