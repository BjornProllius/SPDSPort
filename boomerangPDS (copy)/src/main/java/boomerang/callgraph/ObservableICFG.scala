...
trait ObservableICFG[N, M] {
  ...
  def addCalleeListener(listener: Nothing): Unit

  /** Registers a listener that will be notified whenever a caller is added. */
  def addCallerListener(listener: Nothing): Unit
  ...
  def getStartPointsOf(callee: M): Nothing

  def getEndPointsOf(flowReaches: M): Nothing

  def computeFallback(): Unit

  def addEdges(e: Nothing): Unit
}